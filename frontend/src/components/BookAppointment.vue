...
<template>
  ...
  export default {
    setup() {
      ...
      return {
        // Use a computed property to make the severity level reactive
        severity: severityStore.severity,
      };
    },
    data() {
      return {
      ...
      async submitAppointment() {
        try {
          const response = await axios.post('http://localhost:8001/api/appointments/', {
            ...
            severity: this.severity, // Severity is accessed directly from the store
          });
